...
const uri =
  "mongodb+srv://meherab:lN6OlYa5jSDfwpsF@cluster0.jcgcmli.mongodb.net/?retryWrites=true&w=majority";
...
async function run() {
  try {
    ...
    const cartCollection = client.db("nextDB").collection("cart");
    const enrolledCollection = client.db("nextDB").collection("enrolled");
    const paymentCollection = client.db("nextDB").collection("payments");
    const appliedCollection = client.db("nextDB").collection("applied");
...
app.get("/", (req, res) => {
  res.send("Next Language is running");
});
